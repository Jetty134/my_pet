...
@bot.message_handler(commands=['math'])
    def set_reminder(message):
        bot.send_message(message.chat.id, f'Привет! Введи первое число:')
        bot.register_next_step_handler(message, step2)

    def step2(message):
        n1 = message.text
        bot.send_message(message.chat.id, 'Введи второе число:')
        bot.register_next_step_handler(message, step3, r)

    def step3(message, r):
        n2 = message.text
        bot.send_message(message.chat.id, 'Введи в:')
        bot.register_next_step_handler(message, step3, r)
bot.polling()
'''
import telebot
...
def start(message):
    ...
    item10 = types.KeyboardButton('/history')
    item11 = types.KeyboardButton('/startplay')
    item12 = types.KeyboardButton('/math')
    keyboard.add(item1, item2 ,item3 ,item4 ,item5 ,item6 ,item7 ,item8 ,item9, item10, item11, item12)
    bot.send_message(message.chat.id, 'Выбери команду.', reply_markup=keyboard)
...
def r(message):
    ...
    @bot.message_handler(commands=['math'])
    def set_reminder(message):
        bot.send_message(message.chat.id, f'Привет! Введите первое число:')
        bot.register_next_step_handler(message, step2)

    def step2(message):
        a = message.text
        bot.send_message(message.chat.id, 'Введите второе число:')
        bot.register_next_step_handler(message, step3, a)

    def step3(message, a):
        b = message.text
        bot.send_message(message.chat.id, 'Введите знак:')
        bot.register_next_step_handler(message, step4, a, b)

    def step4(message,a,b):
        c = message.text
        if c == "+":
            bot.send_message(message.chat.id,f"{a} {c} {b} = {a + b}")
        elif c == "*":
            bot.send_message(message.chat.id,f"{a} {c} {b} = {a * b}")
        elif c == "-":
            bot.send_message(message.chat.id,f"{a} {c} {b} = {a - b}")
            ...
            bot.send_message(message.chat.id,"вы ввели неправильный знак!")
